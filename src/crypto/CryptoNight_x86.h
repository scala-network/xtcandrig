...
inline void cryptonight_hash(const uint8_t *__restrict__ input, size_t size, void *__restrict__ output, cryptonight_ctx *__restrict__ ctx)
{
    ...
    keccak(static_cast<const uint8_t*>(input), (int) size, ctx->state0, 200);
        ...
        uint64_t tweak1_2 = (*reinterpret_cast<const uint64_t*>(input + 35) ^ *(reinterpret_cast<const uint64_t*>(ctx->state0) + 24));


    #else

        uint64_t tweak1_2 = 0;
    memcpy(&tweak1_2, input + 35, sizeof tweak1_2);
    tweak1_2 ^= *(reinterpret_cast<const uint64_t*>(ctx->state0) + 24);
    ...
    for (size_t i = 0; i < ITERATIONS; i++) {
        ...
        ((uint64_t*)&l0[idx0 & MASK])[1] = ah0 ^ tweak1_2;
...
template<size_t ITERATIONS, size_t MEM, size_t MASK, bool SOFT_AES>
inline void cryptonight_double_hash(const uint8_t *__restrict__ input, size_t size, void *__restrict__ output, struct cryptonight_ctx *__restrict__ ctx)
{
     if (size < 43) {
        memset(output, 0, 64);
        return;
    ...
    keccak((const uint8_t *) input + size, (int) size, ctx->state1, 200);
        ...
        uint64_t tweak1_2_0 = (*reinterpret_cast<const uint64_t*>(input + 35) ^ *(reinterpret_cast<const uint64_t*>(ctx->state0) + 24));

        uint64_t tweak1_2_1 = (*reinterpret_cast<const uint64_t*>(input + 35) ^ *(reinterpret_cast<const uint64_t*>(ctx->state1) + 24));
    #else

        uint64_t tweak1_2_0 = 0;
    memcpy(&tweak1_2_0, input + 35, sizeof tweak1_2_0);
    tweak1_2_0 ^= *(reinterpret_cast<const uint64_t*>(ctx->state0) + 24);
    uint64_t tweak1_2_1 = 0;
    memcpy(&tweak1_2_1, input + 35, sizeof tweak1_2_1);
    tweak1_2_1 ^= *(reinterpret_cast<const uint64_t*>(ctx->state1) + 24);
